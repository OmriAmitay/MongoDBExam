...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
    	...
    	Document postDoc = new Document("permalink", permalink);
    	
    	Document post = postsCollection.find(postDoc).first();

	    List<Document> comments = (List<Document>) post.get("comments");
	    Document comment = comments.get(ordinal);
	    int numlike = (Integer) comment.get("num_likes");
	    comment.put("num_likes", numlike + 1);
    	    
    	post.put("comments", comments);
    	    
        postsCollection.updateOne(Filters.eq("permalink", permalink), new Document("$set", post));
